...
-- Vārds Uzvārds - Artūrs Skrebutens
...
-- 4. Atlasīt pilsētas kuras ir vecākas par 100 gadiem (WHERE)ALTER
SELECT title,year FROM cities WHERE year>100;
-- 5. Atlasīt pilsētas ar mazāk vai vienādas ar 3501 iedzīvotāju skaitu (WHERE)
SELECT title,population FROM cities WHERE population <= 3501;
-- 6. Atlasīt top 10 lielākas pilsētas pēc iedzīvotāju skaita (LIMIT, ORDER BY)
SELECT title,population FROM cities ORDER BY population DESC LIMIT 10;
-- 7. Saskaitīt kopējo latvijas pilsētu skaitu (COUNT)
SELECT COUNT(title) FROM cities;
-- 8. Atlasīt unikālus novada nosaukumus (DISTINCT)ALTER
